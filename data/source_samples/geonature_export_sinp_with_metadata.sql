...
WITH af_actors AS (
    SELECT cafa.id_acquisition_framework,
           json_build_object('type_role',
                             ...
                             'cd_nomenclature_actor_role', tn.cd_nomenclature, 'name', coalesce(borg.nom_organisme,
                                                                                                ((tro.nom_role::TEXT || ' '::TEXT) || tro.prenom_role::TEXT)::CHARACTER VARYING),
    ...
    FROM gn_meta.cor_acquisition_framework_actor cafa
     ...
     af AS (
         SELECT taf.id_acquisition_framework,
                jsonb_build_object('uuid', taf.unique_acquisition_framework_id, 'name', taf.acquisition_framework_name,
                                   'desc', taf.acquisition_framework_desc, 'start_date',
                                   taf.acquisition_framework_start_date, 'end_date', taf.acquisition_framework_end_date,
                                   ...
                                   taf.ecologic_or_geologic_target, 'actors', json_agg(af_actors.json_data)) AS af_data
         ...
         SELECT cda.id_dataset,
                json_build_object('type_role',
                                  ...
                                  'cd_nomenclature_actor_role', tn.cd_nomenclature, 'name', coalesce(borg.nom_organisme,
                                                                                                     ...
                                                                                                     coalesce(borg.email_organisme, tro.email))) AS json_data
