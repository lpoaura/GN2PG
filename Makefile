...
test: # non-functional
	...
	poetry install
	poetry run pytest --user=${GEONATURE_USER} --password=${GEONATURE_PASSWORD} --url=${GEONATURE_URL} --db-user=dbuser --db-password=dbpass --db-port=5234 --data-export-id=${GEONATURE_DATA_EXPORT_ID} --nb-threads=${GEONATURE_NB_THREADS} tests
