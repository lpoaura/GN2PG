...
BEGIN
    INSERT INTO
        gn_meta.t_acquisition_frameworks ( unique_acquisition_framework_id
...
BEGIN
    ...
    SELECT
        cast(new.item #>> '{id_perm_grp_sinp}' AS UUID)
        INTO the_unique_id_sinp_grp;
    SELECT
        gn2pg_import.fct_c_get_or_insert_source(new.source)
        INTO the_id_source;
    --     id_module                                INT;
    SELECT
        new.item #>> '{id_synthese}'
        INTO the_entity_source_pk_value;
    SELECT
        gn2pg_import.fct_c_get_or_insert_basic_af_from_uuid_name(cast(new.item #>> '{ca_uuid}' AS UUID),
                                                                 new.item #>> '{ca_nom}')
        INTO the_id_af;
    SELECT
        gn2pg_import.fct_c_get_or_insert_basic_dataset_from_uuid_name(cast(new.item #>> '{jdd_uuid}' AS UUID),
                                                                      new.item #>> '{jdd_nom}', the_id_af)
        INTO the_id_dataset;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('NAT_OBJ_GEO', new.item #>> '{nature_objet_geo}')
        INTO the_id_nomenclature_geo_object_nature;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('TYP_GRP', new.item #>> '{type_regroupement}')
        INTO the_id_nomenclature_grp_typ;
    SELECT
        new.item #>> '{methode_regroupement}'
        INTO the_grp_method;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('METH_OBS', new.item #>> '{technique_obs}')
        INTO the_id_nomenclature_obs_technique;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STATUT_BIO', new.item #>> '{statut_biologique}')
        INTO the_id_nomenclature_bio_status;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('ETA_BIO', new.item #>> '{etat_biologique}')
        INTO the_id_nomenclature_bio_condition;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('NATURALITE', new.item #>> '{naturalite}')
        INTO the_id_nomenclature_naturalness;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('PREUVE_EXIST', new.item #>> '{preuve_existante}')
        INTO the_id_nomenclature_exist_proof;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STATUT_VALID', new.item #>> '{statut_validation}')
        INTO the_id_nomenclature_valid_status;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('NIV_PRECIS', new.item #>> '{precision_diffusion}')
        INTO the_id_nomenclature_diffusion_level;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STADE_VIE', new.item #>> '{stade_vie}')
    ...
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('OBJ_DENBR', new.item #>> '{objet_denombrement}')
        INTO the_id_nomenclature_obj_count;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('TYP_DENBR', new.item #>> '{type_denombrement}')
        INTO the_id_nomenclature_type_count;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('SENSIBILITE', new.item #>> '{niveau_sensibilite}')
        INTO the_id_nomenclature_sensitivity;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STATUT_OBS', new.item #>> '{statut_observation}')
        INTO the_id_nomenclature_observation_status;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('DEE_FLOU', new.item #>> '{floutage_dee}')
        INTO the_id_nomenclature_blurring;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STATUT_SOURCE', new.item #>> '{statut_source}')
        INTO the_id_nomenclature_source_status;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('TYP_INF_GEO', new.item #>> '{type_info_geo}')
        INTO the_id_nomenclature_info_geo_type;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('OCC_COMPORTEMENT', new.item #>> '{comportement}')
        INTO the_id_nomenclature_behaviour;
    SELECT
        gn2pg_import.fct_c_get_id_nomenclature_from_label('STAT_BIOGEO', new.item #>> '{statut_biogeo}')
        INTO the_id_nomenclature_biogeo_status;
    SELECT
        new.item #>> '{reference_biblio}'
    ...
    SELECT
        ...
        INTO the_nom_cite;
    SELECT
        new.item #>> '{version_taxref}'
        INTO the_meta_v_taxref;
    SELECT
        new.item #>> '{numero_preuve}'
        INTO the_sample_number_proof;
    SELECT
        new.item #>> '{preuve_numerique}'
        INTO the_digital_proof;
    SELECT
        new.item #>> '{preuve_non_numerique}'
        INTO the_non_digital_proof;
    SELECT
        new.item #>> '{altitude_min}'
        INTO the_altitude_min;
    SELECT
        new.item #>> '{altitude_max}'
        INTO the_altitude_max;
    SELECT
        new.item #>> '{profondeur_min}'
        INTO the_depth_min;
    SELECT
        new.item #>> '{profondeur_max}'
        INTO the_depth_max;
    SELECT
        new.item #>> '{nom_lieu}'
        INTO the_place_name;
    SELECT
        st_setsrid(st_geomfromtext(new.item #>> '{wkt_4326}'), 4326)
        INTO _the_geom_4326;
    SELECT
        st_centroid(_the_geom_4326)
        INTO _the_geom_point;
    SELECT
        st_transform(_the_geom_4326, _local_srid)
        INTO _the_geom_local;
    SELECT
        cast(new.item #>> '{precision}' AS INT)
        INTO the_precision;
    SELECT
        cast(new.item #>> '{date_debut}' AS DATE)
        INTO the_date_min;
    SELECT
        cast(new.item #>> '{date_fin}' AS DATE)
        INTO the_date_max;
    SELECT
        new.item #>> '{validateur}'
        INTO the_validator;
    SELECT
        new.item #>> '{comment_validation}'
        INTO the_validation_comment;
    SELECT
        new.item #>> '{observateurs}'
        INTO the_observers;
    SELECT
        new.item #>> '{determinateur}'
        INTO the_determiner;
    SELECT
        NULL
    ...
    SELECT
        new.item #>> '{comment_releve}'
        INTO the_comment_context;
    SELECT
        new.item #>> '{comment_occurence}'
    ...
    --     SELECT
--         new.item #>> '{date_creation}'
--         INTO the_meta_create_date;
--     SELECT
--         new.item #>> '{date_modification}'
...
BEGIN
    ...
    DELETE
        FROM
            gn_synthese.synthese
        WHERE
            (old.item #>> '{id_synthese}', gn2pg_import.fct_c_get_or_insert_source(old.source)) = (synthese.entity_source_pk_value, synthese.id_source);
    IF NOT found THEN
        RETURN NULL;
    END IF;
    RETURN old;
